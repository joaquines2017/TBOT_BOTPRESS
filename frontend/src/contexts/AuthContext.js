...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      if (token) {
        ...
        try {
          console.log(
            '🔍 [AuthContext] Intentando verificar token en: https://192.168.100.254:3003/api/auth/verify',
          )
          const response = await axios.get('https://192.168.100.254:3003/api/auth/verify')
  ...
  const login = async (credentials) => {
    try {
      ...
      const response = await axios.post('https://192.168.100.254:3003/api/auth/login', credentials)
      ...
      try {
        const verifyResponse = await axios.get('https://192.168.100.254:3003/api/auth/verify')
