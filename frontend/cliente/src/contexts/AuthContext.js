...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      if (token) {
        ...
        try {
          console.log(
            '🔍 [AuthContext] Intentando verificar token en: http://tbot_backend:3003/api/auth/verify',
          )
          const response = await axios.get('http://tbot_backend:3003/api/auth/verify')
          ...
          if (
            ...
            alert(
              '⚠️ No se puede conectar al servidor. Verifique que el servidor esté ejecutándose en http://tbot_backend:3003',
            )
            localStorage.clear()
            setUser(null)
  ...
  const login = async (credentials) => {
    try {
      ...
      const response = await axios.post('http://tbot_backend:3003/api/auth/login', credentials)
      ...
      try {
        const verifyResponse = await axios.get('http://tbot_backend:3003/api/auth/verify')
