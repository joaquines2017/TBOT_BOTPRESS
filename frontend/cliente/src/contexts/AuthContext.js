  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      if (token) {
        ...
        try {
          console.log(
            '🔍 [AuthContext] Intentando verificar token en: http://localhost:3003/api/auth/verify',
          )
          ...
          if (
            ...
            alert(
              '⚠️ No se puede conectar al servidor. Verifique que el servidor esté ejecutándose en http://localhost:3003',
            )
            localStorage.clear()
            setUser(null)
