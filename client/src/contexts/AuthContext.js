...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      console.log('🔄 [AuthContext] Inicializando autenticación:', { token: token ? 'presente' : 'ausente' })
      
      if (token) {
        axios.defaults.headers.common['Authorization'] = `Bearer ${token}`
        
        try {
          console.log('🔍 [AuthContext] Intentando verificar token en: http://localhost:3003/api/auth/verify')
          ...
          if (error.code === 'ECONNREFUSED' || error.message.includes('Network Error') || !error.response) {
            ...
            alert('⚠️ No se puede conectar al servidor. Verifique que el servidor esté ejecutándose en http://localhost:3003')
            localStorage.clear()
            setUser(null)
            ...
            if (error.response.data?.details?.includes('observaciones') || 
  ...
  const login = async (credentials) => {
    try {
      ...
      try {
        const verifyResponse = await axios.get('http://localhost:3003/api/auth/verify')
